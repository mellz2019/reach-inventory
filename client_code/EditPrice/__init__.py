...
class EditPrice(EditPriceTemplate):
  ...
  def handle_confirm_price(self):
    ...
    if self.regular_price_checkbox.checked:
      ...
      self.confirm_price_button.enabled = False
      self.cancel_button.enabled = False
      Globals.order = Globals.edit_product_in_order_by_id(Globals.product['id'], 'Edited Price', Globals.product['fields']['Lowest Price Lookup'][0])
      ...
      update_product = {
      "Edited Price": Globals.product['fields']['Lowest Price Lookup'][0],
      ...
      if not Globals.is_valid_currency(self.custom_price_textfield.text):
        ...
        return
      if float(self.custom_price_textfield.text) < Globals.product['fields']['Lowest Price Lookup'][0]:
        alert(f"Custom price (${Globals.round_to_decimal_places(self.custom_price_textfield.text, 2)}) is lower than the lowest price (${Globals.round_to_decimal_places(Globals.product['fields']['Lowest Price Lookup'][0], 2)}) accepted for this product. Please enter a higher amount.")
        self.confirm_price_button.enabled = True
        self.cancel_button.enabled = True
        self.confirm_price_button.text = 'Confirm Price'
        return
  ...
  def custom_price_textfield_change(self, **event_args):
    ...
    if self.custom_price_textfield.text != None:
      if self.custom_price_textfield.text < Globals.product['fields']['Lowest Price Lookup'][0]:
        self.confirm_price_button.enabled = False
      else:
        self.confirm_price_button.enabled = True
